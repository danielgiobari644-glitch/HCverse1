...
function attachViewSpecificListeners() {
  switch (state.currentView) {
    ...
    case 'downloads':
      ...
      // Browser PWA installations trigger click proxy listener
      document.getElementById('pwa-trigger-direct-install')?.addEventListener('click', () => {
        const installBtn = document.getElementById('pwa-prompt-install-btn');
        if (installBtn) {
          installBtn.click();
        } else {
          // If browser didn't supply prompt event (e.g. mobile Safari), provide instructions inline
          const alertDesc = document.createElement('div');
          alertDesc.className = "fixed top-24 left-1/2 -translate-x-1/2 bg-black/90 p-6 border border-purple-500 rounded-3xl text-sm text-white max-w-sm shadow-2xl z-50 text-center animate-fade-in";
          alertDesc.innerHTML = `
            <strong class="block mb-2 text-primary font-['Space_Grotesk'] text-sm">💡 Installation Instructions</strong>
            <p class="leading-relaxed text-muted-foreground mb-4">
              If your browser didn't showcase an install prompt yet: Tap your browser options menu and click <b class="text-white">"Install"</b> or <b class="text-white">"Add to Home Screen"</b>. iOS users should select <b class="text-white">Share</b> &rarr; <b class="text-white">Add to Home Screen</b> in Safari.
            </p>
            <button id="close-manual-guide-btn" class="bg-primary/20 text-purple-300 px-4 py-2 rounded-xl border border-purple-500/10 font-bold hover:bg-primary/30 w-full cursor-pointer">Closed</button>
          `;
          document.body.appendChild(alertDesc);
          document.getElementById('close-manual-guide-btn')?.addEventListener('click', () => alertDesc.remove());
        }
      });
...
// --------------------------------------------------------------------------
// PROGRESSIVE WEB APP (PWA) INSTALL CONTROLLER & CACHING INTEGRATION
// --------------------------------------------------------------------------
(function initPwaController() {
  let deferredPrompt = null;

  // Stash standard beforeinstallprompt triggers
  ...
  const banner = document.getElementById('pwa-download-prompt');
  const closeBtn = document.getElementById('pwa-prompt-close');
  const dismissBtn = document.getElementById('pwa-prompt-dismiss-btn');
  const installBtn = document.getElementById('pwa-prompt-install-btn');
  const promptText = document.getElementById('pwa-prompt-text');
  ...
  function showPromptIfEligible() {
    // If already downloaded or dismissed, skip the prompt completely
    const dismissed = localStorage.getItem('hcverse-pwa-dismissed') === 'true';
    if (isStandalone() || dismissed) {
      return;
    }
    // Present the floating invitation prompt
    if (banner) {
      banner.classList.remove('hidden');
    }
  }

  // Trigger non-deferred initial check for compatibility (e.g. iOS or manual)
  setTimeout(() => {
    showPromptIfEligible();
  }, 3500);

  // Close and save user's explicit preference to avoid constant pestering
  function dismissPrompt() {
    if (banner) {
      banner.classList.add('hidden');
    }
    localStorage.setItem('hcverse-pwa-dismissed', 'true');
  }
  ...
  installBtn?.addEventListener('click', async () => {
    ...
    if (isiOS) {
      // Prompt step-by-step instructions inline in the card for iOS
      if (promptText) {
        promptText.innerHTML = `
          <strong class="text-white block mb-1">To Download on iOS Safari:</strong>
          1. Tap the <b class="text-purple-400">Share button</b> (📤) at the bottom.<br/>
          2. Scroll down and choose <b class="text-purple-400">Add to Home Screen</b>.<br/>
          3. Tap <b class="text-purple-400">Add</b> on the top right.
        `;
      }
      if (installBtn) {
        installBtn.textContent = 'Ready to study';
    ...
    if (deferredPrompt) {
      ...
      console.log(`[PWA] User choice outstanding outcome: ${outcome}`);
      deferredPrompt = null;
      dismissPrompt();
    } else {
      ...
      if (promptText) {
        promptText.innerHTML = `
          To install on your browser: select the browser menu (<b class="text-purple-400">⋮ / ⋯</b>) and tap <b class="text-purple-300">Install/Save App</b>.
        `;
      }
    }
  });
